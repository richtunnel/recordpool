...
interface CartPageProps {
    removeDrawer: (transition: boolean) => void;
  }
...
export default function CartPage({removeDrawer}: CartPageProps) {
  const [cart, setCart] = useState<CartItem[]>([]);
  const [totalPrice, setTotalPrice] = useState<number>(0);
  const [transitionCheckout, setTransitionCheckout] = useState<boolean>(false);
...
const closeDrawer = () => {
  ...
  return (
    <Box className="main-cart" sx={{ padding: "2rem", margin: "auto" }}>
            ...
            {cart.map((item) => (
              <Row className="cart-table-rows" key={item.id}>
                ...
                <Box className="cart-qty-box" sx={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                    <IconButton
                      onClick={() => handleChangeQuantity(item.id, Math.max(1, item.qty - 1))}
                    >
                      <IoIosRemoveCircle />
                    </IconButton>
                    <TextField
                    className='cart-qty-field'
                      value={item.qty || 0}
                      onChange={(e) =>
                        handleChangeQuantity(item.id, Math.max(1, Number(e.target.value)))
                      }
                      type="number"
                      size="small"
                      inputProps={{
                    ...
                    <div className="d-flex ml-5">
                        <span>
                            <button onClick={() => handleRemove(item.id)}><IoTrashOutline color="#ff8989" />
                            </button>
                            </span>
                    </div>
                  </Box>
      ...
      <br />
      <br />
      <Divider className='cart-divider' sx={{ my: 2, color: "#2b2b39"}} />
      <Box className="cart-box-total" sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <Typography className='h4 font-bold' variant="h3">Total: ${totalPrice.toFixed(2)}</Typography>
        <NavLink onClick={closeDrawer} to="checkout" className='custom-button-cart'>
          Complete Purchase
        </NavLink>
      </Box>
    </Box>
